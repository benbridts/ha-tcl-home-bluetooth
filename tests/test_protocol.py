...
class TestBuildFrame:
    ...
    def test_build_frame_basic(self):
        ...
        expected_checksum = TCLSoundbarProtocol.calculate_checksum(frame[:-1])
    ...
    def test_build_frame_with_data(self):
        ...
        command = 0x02
        data = bytes([0x32])
        frame = TCLSoundbarProtocol.build_frame(command, data)
    ...
    def test_build_frame_length_calculation(self):
        ...
        frame = TCLSoundbarProtocol.build_frame(0x10, data)
...
class TestChecksum:
    ...
    def test_calculate_checksum(self):
        ...
        assert TCLSoundbarProtocol.calculate_checksum(bytes([0x01, 0x02, 0x03])) == 0x00
        # XOR of [0xFF] = 0xFF
        assert TCLSoundbarProtocol.calculate_checksum(bytes([0xFF])) == 0xFF
        # XOR of [0xAA, 0x00, 0x05, 0x02] = 0xAA ^ 0x00 ^ 0x05 ^ 0x02 = 0xAD
        assert TCLSoundbarProtocol.calculate_checksum(bytes([0xAA, 0x00, 0x05, 0x02])) == 0xAD

    def test_checksum_roundtrip(self):
        """Build a frame and verify parse_frame accepts it (checksum is valid)."""
        frame = TCLSoundbarProtocol.build_frame(0x06, bytes([0x01]))
        result = TCLSoundbarProtocol.parse_frame(frame)
...
class TestParseFrame:
    """Tests for TCLSoundbarProtocol.parse_frame."""
    ...
    def test_parse_frame_valid(self):
        ...
        frame = TCLSoundbarProtocol.build_frame(original_command, original_data)
    ...
    def test_parse_frame_too_short(self):
        ...
        assert TCLSoundbarProtocol.parse_frame(bytes([0xAA, 0x00, 0x05])) is None
        assert TCLSoundbarProtocol.parse_frame(bytes([0xAA])) is None
        assert TCLSoundbarProtocol.parse_frame(b"") is None

    def test_parse_frame_invalid_header(self):
        """First byte not 0xAA returns None."""
        frame = TCLSoundbarProtocol.build_frame(0x02, bytes([0x32]))
        # Corrupt the header
        bad_frame = bytes([0xBB]) + frame[1:]
        assert TCLSoundbarProtocol.parse_frame(bad_frame) is None
    ...
    def test_parse_frame_incomplete_length(self):
        ...
        frame = TCLSoundbarProtocol.build_frame(0x02, bytes([0x01, 0x02, 0x03]))
        # Truncate the frame (remove last 2 bytes so it's shorter than declared)
        truncated = frame[:-2]
        assert TCLSoundbarProtocol.parse_frame(truncated) is None
...
class TestHelperMethods:
    ...
    def test_build_set_volume(self):
        ...
        frame = TCLSoundbarProtocol.build_set_volume(50)
        result = TCLSoundbarProtocol.parse_frame(frame)
        assert result is not None
        command, data = result
    ...
    def test_build_set_volume_clamped(self):
        ...
        frame_high = TCLSoundbarProtocol.build_set_volume(150)
        result_high = TCLSoundbarProtocol.parse_frame(frame_high)
        assert result_high is not None
        _, data_high = result_high
        assert data_high == bytes([100])

        frame_low = TCLSoundbarProtocol.build_set_volume(-10)
        result_low = TCLSoundbarProtocol.parse_frame(frame_low)
    ...
    def test_build_set_power_on(self):
        ...
        frame = TCLSoundbarProtocol.build_set_power(True)
        result = TCLSoundbarProtocol.parse_frame(frame)
        assert result is not None
        command, data = result
    ...
    def test_build_set_power_off(self):
        ...
        frame = TCLSoundbarProtocol.build_set_power(False)
        result = TCLSoundbarProtocol.parse_frame(frame)
        assert result is not None
        command, data = result
        assert command == CMD_SET_POWER
    ...
    def test_build_set_mute_on(self):
        ...
        frame = TCLSoundbarProtocol.build_set_mute(True)
        result = TCLSoundbarProtocol.parse_frame(frame)
        assert result is not None
        command, data = result
    ...
    def test_build_set_mute_off(self):
        ...
        frame = TCLSoundbarProtocol.build_set_mute(False)
        result = TCLSoundbarProtocol.parse_frame(frame)
        assert result is not None
        command, data = result
        assert command == CMD_SET_MUTE
...
class TestTDataMerger:
    ...
    def test_merger_single_packet(self):
        ...
        merger = TDataMerger()
        frame = TCLSoundbarProtocol.build_frame(0x84, bytes([0x32]))
    ...
    def test_merger_multi_packet(self):
        ...
        merger = TDataMerger()
        frame = TCLSoundbarProtocol.build_frame(0x02, bytes([0x01, 0x02, 0x03, 0x04, 0x05]))
    ...
    def test_merger_reset(self):
        ...
        merger = TDataMerger()
        frame = TCLSoundbarProtocol.build_frame(0x02, bytes([0x50]))
